...
sources:
  ...
  - "[[wiki/summaries/how-scientists-use-claude-code.md]]"
...
**Claude Code** is Anthropic's command-line agentic coding tool that runs in a terminal or inside a code editor (VS Code, Cursor). Unlike browser-based chatbots, Claude Code operates directly on the user's local filesystem -- reading, writing, and executing files -- and can integrate with external services via the [[wiki/concepts/mcp-protocol.md|Model Context Protocol]]. It is the primary tool discussed across the majority of sources in this wiki.
...
**Blattman** ([[wiki/summaries/claude-code-newbies.md]]) provides the primary onboarding guide for non-technical users. He frames the terminal as the biggest hurdle and systematically addresses it, explaining the three operating modes and structuring setup as a five-step sequence: install Claude Code, optionally install VS Code, create a [[wiki/concepts/claude-md-files.md|CLAUDE.md]] file, and configure [[wiki/concepts/mcp-protocol.md|MCP integrations]]. The end result is an AI system with email, calendar, and document integrations that improves over time.

**Sant'Anna** ([[wiki/summaries/my-claude-code-setup.md]]) presents the most sophisticated academic setup: a "contractor mode" orchestrator with 10 specialized agents, an adversarial critic-fixer loop (5 rounds, hard role separation), 22 slash commands, and 18 context-aware rules. The template covers the full academic lifecycle (slides, papers, data analysis, replication packages) and has been adopted by 15+ research groups. This represents the ceiling of what a well-configured Claude Code environment can do.

**Yang** ([[wiki/summaries/how-scientists-use-claude-code.md]]) provides the only empirical measurement of adoption among scientists. The data shows that economists are the highest adopters, that adoption is still very early (significant first-mover advantage available), and that enterprise access remains a barrier AI labs have not cracked.

**Mollick** ([[wiki/summaries/claude-code-what-comes-next.md]]) provides the conceptual framework for non-programmers, explaining compacting, skills, sub-agents, and MCP as the "magic tricks" that make sustained autonomous work possible. His demonstration -- building a functional e-commerce website from a single prompt in 74 minutes -- illustrates the ceiling of autonomous capability.

**Van Horn** ([[wiki/summaries/claude-code-hacks.md]]) represents the power-user perspective: 4-6 parallel Claude Code sessions, voice input, bypass permissions, and audio notifications. His core principle -- "plan first, always" -- means every idea immediately becomes a plan.md file before any implementation begins.

**Panjwani** ([[wiki/summaries/ai-agents-econ-research.md]]) provides budget-conscious guidance for economists, noting that at $20/month Codex may be preferable to Claude Code, but at $200/month Claude Code's capabilities justify the investment since learning speed is proportional to usage.
...
- **Configure a CLAUDE.md**: Without persistent configuration, every session starts cold. A [[wiki/concepts/claude-md-files.md|CLAUDE.md file]] gives Claude context about your identity, projects, and preferences from the first interaction.
...
- [[wiki/summaries/claude-code-newbies.md]] -- Blattman's setup guide for non-technical users
- [[wiki/summaries/my-claude-code-setup.md]] -- Sant'Anna's comprehensive academic workflow template
- [[wiki/summaries/how-scientists-use-claude-code.md]] -- Yang's empirical adoption data
- [[wiki/summaries/claude-code-what-comes-next.md]] -- Mollick's explanation of the agentic harness
- [[wiki/summaries/claude-code-hacks.md]] -- Van Horn's power-user techniques
- [[wiki/summaries/ai-agents-econ-research.md]] -- Panjwani's practical guide for economists
